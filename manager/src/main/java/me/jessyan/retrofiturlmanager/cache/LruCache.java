...
/**
 * ================================================ LRU 即 Least Recently Used, 最近最少使用, 也就是说, 当缓存满了,
 * 会优先淘汰那些最近最不常访问的数据 此种缓存策略为框架默认提供, 可自行实现其他缓存策略, 如磁盘缓存
 *
 * @see Cache Created by JessYan on 25/09/2017 16:57 <a
...
public class LruCache<K, V> implements Cache<K, V> {
  ...
  /**
   * 设置一个系数应用于当时构造函数中所传入的 size, 从而得到一个新的 {@link #maxSize} 并会立即调用 {@link #evict} 开始清除满足条件的条目
   *
   * @param multiplier 系数
   */
  ...
  /**
   * 返回每个 {@code item} 所占用的 size,默认为1,这个 size 的单位必须和构造函数所传入的 size 一致 子类可以重写这个方法以适应不同的单位,比如说 bytes
   *
  ...
  /**
   * 将 {@code key} 和 {@code value} 以条目的形式加入缓存,如果这个 {@code key} 在缓存中已经有对应的 {@code value} 则此 {@code
   * value} 被新的 {@code value} 替换并返回,如果为 {@code null} 说明是一个新条目
   *
   * <p>如果 {@link #getItemSize} 返回的 size 大于或等于缓存所能允许的最大 size, 则不能向缓存中添加此条目 此时会回调 {@link
   * #onItemEvicted(Object, Object)} 通知此方法当前被驱逐的条目
   *
  ...
  /**
   * 移除缓存中这个 {@code key} 所对应的条目,并返回所移除条目的 {@code value} 如果返回为 {@code null} 则有可能时因为这个 {@code key} 对应的
   * {@code value} 为 {@code null} 或条目不存在
   *
