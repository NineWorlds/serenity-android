...
/**
 * ================================================ 超级解析器 超级模式属于高级模式的加强版, 优先级高于高级模式, 在高级模式中, 需要传入一个
 * BaseUrl (您传入 Retrofit 的 BaseUrl) 作为被替换的基准 如这个传入的 BaseUrl 为 "https://www.github.com/wiki/part"
 * (PathSize = 2), 那框架会将所有需要被替换的 Url 中的 域名 以及 域名 后面的前两个 pathSegments 使用您传入 {@link
 ...
 *
 * <p>超级模式也需要手动开启, 但与高级模式不同的是, 开启超级模式并不需要调用 API, 只需要在 Url 中加入 {@link
 * RetrofitUrlManager#IDENTIFICATION_PATH_SIZE} + PathSize
 *
 * <p>替换规则如下: 1. 旧 URL 地址为 https://www.github.com/wiki/part#baseurl_path_size=1,
 * #baseurl_path_size=1 表示其中 BaseUrl 为 https://www.github.com/wiki 您调用 {@link
 * RetrofitUrlManager#putDomain(String, String)}方法传入的 URL 地址是 https://www.google.com/api
 * 经过本解析器解析后生成的新 URL 地址为 http://www.google.com/api/part
 *
 * <p>2. 旧 URL 地址为 https://www.github.com/wiki/part#baseurl_path_size=1, #baseurl_path_size=1 表示其中
 * BaseUrl 为 https://www.github.com/wiki 您调用 {@link RetrofitUrlManager#putDomain(String,
 * String)}方法传入的 URL 地址是 https://www.google.com 经过本解析器解析后生成的新 URL 地址为 http://www.google.com/part
 *
 * <p>3. 旧 URL 地址为 https://www.github.com/wiki/part#baseurl_path_size=0, #baseurl_path_size=0 表示其中
 * BaseUrl 为 https://www.github.com 您调用 {@link RetrofitUrlManager#putDomain(String, String)}方法传入的
 * URL 地址是 https://www.google.com/api 经过本解析器解析后生成的新 URL 地址为 http://www.google.com/api/wiki/part
 *
 * <p>4. 旧 URL 地址为 https://www.github.com/wiki/part/issues/1#baseurl_path_size=3,
 * #baseurl_path_size=3 表示其中 BaseUrl 为 https://www.github.com/wiki/part/issues 您调用 {@link
 * RetrofitUrlManager#putDomain(String, String)}方法传入的 URL 地址是 https://www.google.com/api
 * 经过本解析器解析后生成的新 URL 地址为 http://www.google.com/api/1
 *
 * @see UrlParser Created by JessYan on 2018/6/21 16:41 <a
 *     href="mailto:jess.yan.effort@gmail.com">Contact me</a> <a
...
public class SuperUrlParser implements UrlParser {
  ...
  public HttpUrl parseUrl(HttpUrl domainUrl, HttpUrl url) {
    ...
    HttpUrl httpUrl =
        builder.scheme(domainUrl.scheme()).host(domainUrl.host()).port(domainUrl.port()).build();
