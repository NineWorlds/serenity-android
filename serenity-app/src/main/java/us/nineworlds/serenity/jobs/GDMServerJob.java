  ...
  @Override public void onRun() throws Throwable {
    try {
      ...
      DatagramPacket packet =
          new DatagramPacket(data.getBytes(), data.length(), useMultiCastAddress(), 32414);
      ...
      while (listening) {
        try {
          ...
          if (packetData.contains("HTTP/1.0 200 OK")) {
            ...
            LocalBroadcastManager.getInstance(context).sendBroadcast(packetBroadcast);
          }
          ...
          LocalBroadcastManager.getInstance(context).sendBroadcast(socketBroadcast);
        }
      }
    } catch (IOException e) {
