...
public class ReflectingLayout extends LinearLayout {
   ...
   *
   * @param maxReflectionRatio The size of the reflection in percent
   */
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    if (getReflectionHeight() > 0) {
      ...
      Bitmap flippedBitmap =
          Bitmap.createBitmap(sourceBitmap, 0, 0, sourceBitmap.getWidth(), childBottom, vFlipMatrix,
              false);

      // Create a bitmap to hold just the reflection
      Bitmap fadedBitmap =
          Bitmap.createBitmap(getMeasuredWidth(), reflectionHeight, Bitmap.Config.ARGB_8888);

      Canvas fadeCanvas = new Canvas(fadedBitmap);
      fadeCanvas.drawBitmap(flippedBitmap, 0, 0, null);

      LinearGradient gradient =
          new LinearGradient(0, 0, 0, reflectionHeight, 0x30FFFFFF, 0x00FFFFFF, TileMode.CLAMP);
