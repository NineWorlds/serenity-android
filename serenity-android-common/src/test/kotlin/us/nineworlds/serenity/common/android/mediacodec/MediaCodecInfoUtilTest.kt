...
class MediaCodecInfoUtilTest {
    ...
    fun `getMaxSupportedChannels handles exception gracefully`() {
        ...
        every { codecInfo.isEncoder } returns false
        every { codecInfo.supportedTypes } returns arrayOf("audio/mp4a-latm")
        every { codecInfo.getCapabilitiesForType("audio/mp4a-latm") } throws
    ...
    fun `findCorrectVideoMimeType returns correct mime type`() {
        assertThat(
            MediaCodecInfoUtil.findCorrectVideoMimeType("video/mpeg-4")
        ).isEqualTo("video/mp4")
        assertThat(
            MediaCodecInfoUtil.findCorrectVideoMimeType("video/mpeg4")
        ).isEqualTo("video/mp4v-es")
        assertThat(MediaCodecInfoUtil.findCorrectVideoMimeType("video/h264")).isEqualTo("video/avc")
        assertThat(
            MediaCodecInfoUtil.findCorrectVideoMimeType("video/h263")
        ).isEqualTo("video/3gpp")
        assertThat(
            MediaCodecInfoUtil.findCorrectVideoMimeType("video/mpeg2")
        ).isEqualTo("video/mpeg2")
        assertThat(
            MediaCodecInfoUtil.findCorrectVideoMimeType("video/unknown")
        ).isEqualTo("video/unknown")
    }
    ...
    fun `findCorrectAudioMimeType returns correct mime type`() {
        ...
        assertThat(
            MediaCodecInfoUtil.findCorrectAudioMimeType("audio/aac")
        ).isEqualTo("audio/mp4a-latm")
        assertThat(
            MediaCodecInfoUtil.findCorrectAudioMimeType("audio/unknown")
        ).isEqualTo("audio/unknown")
    }
}
