...
3. **Pre-Drafting Standard & Skill Review:** Before generating the spec, you MUST:
       ...
       - **Standard Review**: Review all standard files in `prompts/agents/` (e.g., `architecture.md`, `ui-standards.md`, `communication-protocol.md`, `TESTING.md`) to ensure the proposed spec aligns with the project's "Golden Standards".
4. **ORGANIZE by Plan:** Create a new subdirectory for the spec at `prompts/plans/<plan_name>/`.
...
- **Simple Tasks (1-2 files, minor logic):** Proceed with standard protocol (Spec -> Permission -> Execute).
